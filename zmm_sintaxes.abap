...
*------------------------------------------------------------------------*
*SINTAXES UTILIZADAS NO PROGRAMA: TABLES, SELECT, DATA, REFRESH, CLEAR, LOOP LINE-SIZE,
*LINE-COUNT, PARAMETERS, SORT,READ TABLE.
*------------------------------------------------------------------------*
...
* O COMANDO PARAMETERS serve para colocar um parametro de seleção PARAMETERS nome_do_parametro LIKE tabela_interna.
* PARAMETERS P_WERKS LIKE T_001W-WERKS.
* O comando SELECT-OPTIONS funciona como o comando PARAMETERS SELECT-OPTIONS variável FOR campo da tabela DEFAULT valor_inicial
